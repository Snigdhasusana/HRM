...
public class Module1 extends UserLibrary
{
	...
	public static void TC03_AddDependenciesAndVerify()
	{
		...
		if(status)
			LogEventWithScreeshot("pass", "Clicked on Myinfo");
		else
			LogEventWithScreeshot("Fail", "Not clicked on Myinfo");
		status = ClickElement(".//*[@id='sidenav']/li[4]/a");
		if(status)
			LogEventWithScreeshot("pass", "Clicked on Dependencies");
		else
			LogEventWithScreeshot("Fail", "Not clicked Dependencies");
		status = ClickElement(".//*[@id='btnAddDependent']");
		if(status)
			LogEventWithScreeshot("pass", "Clicked on Addbtn");
		else
			LogEventWithScreeshot("Fail", "Not clicked Addbtn");
		status = SetText(".//*[@id='dependent_name']", Name);
		if(status)
			LogEventWithScreeshot("pass", "Entered Name");
		else
			LogEventWithScreeshot("Fail", "Not Entered name");
		status = SelectOPtionByText(".//*[@id='dependent_relationshipType']", RelationShip);
		if(status)
			LogEventWithScreeshot("pass", "Selected ");
		else
